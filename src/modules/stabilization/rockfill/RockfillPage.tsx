...
function Sketch({ input }: { input: Input }) {
  ...
  return <svg className="beam-svg" viewBox="0 0 520 260" role="img" aria-label="Desenho Enrocamentos">
    ...
    <text x="82" y="42" fontWeight="700">🪨 Enrocamentos</text>
    <text x="82" y="244">H={input.height} m · L={input.length} m</text>
  </svg>;
}

export function RockfillPage() {
  ...
  return <section className="page">
    <div className="page-title"><h1>🪨 Enrocamentos</h1><span className={result.status.startsWith('Verifica') ? 'status ok' : 'status warning'}>{result.status}</span></div>
    <div className="workspace">
      <div className="workspace-main">
        <Card title="Dados">
          <p className="muted">Quantidades preliminares de enrocamento e camada de proteção.</p>
      ...
      <aside className="result-panel">
        ...
        <p>volume: <strong>{result.quantity.toFixed(2)} m³</strong></p>
