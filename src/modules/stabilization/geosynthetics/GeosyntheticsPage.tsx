...
function Sketch({ input }: { input: Input }) {
  ...
  return <svg className="beam-svg" viewBox="0 0 520 260" role="img" aria-label="Desenho Geossintéticos">
    ...
    <text x="82" y="42" fontWeight="700">🌱 Geossintéticos</text>
    <text x="82" y="244">H={input.height} m · L={input.length} m</text>
  </svg>;
}

export function GeosyntheticsPage() {
  ...
  return <section className="page">
    <div className="page-title"><h1>🌱 Geossintéticos</h1><span className={result.status.startsWith('Verifica') ? 'status ok' : 'status warning'}>{result.status}</span></div>
    <div className="workspace">
      <div className="workspace-main">
        <Card title="Dados">
          <p className="muted">Estimativa de geotêxteis, geogrelhas e sobreposições.</p>
      ...
      <aside className="result-panel">
        ...
        <p>área: <strong>{result.quantity.toFixed(2)} m²</strong></p>
